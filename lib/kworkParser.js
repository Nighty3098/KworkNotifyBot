...
class KworkParser {
  ...
  async getProjects() {
    try {
      ...
      // Ищем данные в window.stateData
  ...
  // Очистка старых проектов (чтобы не накапливались в памяти)
  cleanupOldProjects() {
    // Можно добавить логику для удаления старых проектов
    // Например, хранить только последние 1000 проектов
